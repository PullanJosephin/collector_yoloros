...
void CollectProcess::statusCallback(const actionlib_msgs::GoalStatusArray::ConstPtr& msg)
{
    ...
    if (last_status.status == actionlib_msgs::GoalStatus::SUCCEEDED && arm_state == IDLE) {
        ROS_INFO("导航成功，切换机械臂到工作状态");
        arm_state = WORKING;
      }
  }
...
void CollectProcess::target_pointCb(const sensor_msgs::PointCloudConstPtr &msg)
{
  if (!grasp_running )
  {
    ...
    std::cout<< " Z-Co-ordinate in Camera Frame :" << obj_z << std::endl;
   ...
   else
    {
       ...
       else if(ros::Time::now() - empty_frame_start_time > ros::Duration(5))
       {
...
int main(int argc, char **argv)
{
  ...
  ros::NodeHandle n;
  ros::Publisher joint_states_pub=n.advertise<sensor_msgs::JointState>("/move_group/fake_controller_joint_states",10); //同步关节状态
